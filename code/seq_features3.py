...
with open("test.csv") as test_file:
    ...
    for line in test_file:
        ...
        if count < 300:
            ...
            sequence_features1[int(e), 0] = np.mean(lon)
            sequence_features1[int(e), 1] = np.mean(lat)
            sequence_features1[int(e), 2] = np.mean(R)

            count = 0
            if int(e) % 1000 == 0:
                print "Sequences done : " + e

np.savetxt("sequence_features3.csv", sequence_features1, delimiter=",")
